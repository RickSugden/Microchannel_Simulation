...
void interact_lxwall(int p1)
{
  ...
  if(d2 < r[p1]*r[p1]) //this means if the distance btwn wall and ball is less than the r of ball, theyre touching, so compute
  {
    ...
    fx = kn * diff * rhatx + gamman * vijnx; //kn is in units of N/m (spring constant)
    fy = 0.0; // we continue to assume force in the y direction is zero
    
    fdotrhat = fx * rhatx; //force vector in x direction

    if (fdotrhat > 0.) //if the force is away from the wall set it to zero?
      fdotrhat = 0.;

    else
    if (fdotrhat < 0.) //if the force is into the wall
    {
      vijtx = 0.;
      vijty = vyij;
