...
for i = 1:1%NUMBER_OF_VELOCITY_FILES
    
    file_num = 0;
    file_num = 1300+i;%1199
    file_num = num2str(file_num);
    filename = ['data_output',' ',file_num];
    data =load(filename);
...
%plot the shear force vs velocity
figure
x = [0.0001 0.0002 0.0003 0.0004 0.0005 0.0006 0.00055]; %other values threw errors 
...
newy = y(x_order);
plot(x_sorted,newy);
title('Left Wall Shear Force vs Wall Velocity');
